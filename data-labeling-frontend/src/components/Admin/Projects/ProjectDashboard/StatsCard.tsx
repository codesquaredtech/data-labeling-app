...
function TimeStats(props: any) {
    ...
    return (
        <>
            <h4 className="text-3xl text-neutral-content font-bold">
                {days && <><CountUp duration={countupDuration} end={days} /><span className="text-2xl font-bold">d </span></>}
                {(days || hours) && <><CountUp duration={countupDuration} end={hours || 0} /><span className="text-2xl font-bold">h </span></>}
                {(minutes || seconds) && <><CountUp duration={countupDuration} end={minutes || 0} /><span className="text-2xl font-bold">m </span></>}
                {seconds && <><CountUp duration={countupDuration} end={seconds} /><span className="text-2xl font-bold">s</span></>}
            </h4>
            <div className={`${positiveProgress ? 'text-success' : 'text-error'} text-3xl font-bold`}>
                {positiveProgress && '+'}<CountUp duration={countupDuration} end={progressValue} />

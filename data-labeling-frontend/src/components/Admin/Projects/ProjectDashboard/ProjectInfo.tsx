...
export default function ProjectInfo({ setModalOpen }: { setModalOpen: (open: boolean) => void }) {
  const project = useSelector(projectsSliceSelectors.project);
  ...
  const finishProjectSetup = () => {
    ...
    const { identNumber, title, description, draft } = project;
    dispatch(updateProject({ submitData: { projectId: identNumber, data: { title, description, draft: false } } }));
  };
    ...
    <div className="flex flex-col w-full h-fit mb-8">
      <div className={"flex flex-row justify-between"}>
        <div className="flex items-center gap-2">
          ...
          >
            <h1 className="text-3xl mr-2">{project?.title || "Project name"}</h1>
            {isHovered && <span className="text-sm text-warning">Edit</span>}
          </div>
        </div>
        <div>
          {project?.draft !== false && (
            <button onClick={finishProjectSetup} className="btn btn-sm">
              Finish setup
            </button>
          )}
        </div>
